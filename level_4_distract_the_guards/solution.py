...
"""
Solution for foobar starts here. 
It uses Blossom's algorithm implementation from above.
"""
...
def is_infinite_game(a, b):
    if (a+b) & 1: return True
    if a==b: return False
    a, b = _order(a, b)
    c = gcd(a, b)
    a, b = int(a/c), int(b/c)
    if ((a+b) & (a+b-1)) == 0: return False
    return True

def solution(banana_list):
    n = len(banana_list)
    G = {}
    for i in range(n): G[i] = set()
    for i in range(n):
        for j in range(i+1, n):
            if is_infinite_game(banana_list[i], banana_list[j]):
                G[i].add(j)
                G[j].add(i)

    min_card_matching = matching(G)

    return n - len(min_card_matching)
